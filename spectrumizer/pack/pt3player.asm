; PT3 player for ZX Spectrum, by Sergey Bulba (Vortex Tracker II v1.0, r.7).
; Bundled with spectrumizer to package .pt3 modules into a self-playing tape /
; snapshot. This is Bulba's original player; it is NOT under spectrumizer's MIT
; licence (see LICENSING.md). Only two mechanical changes were made for reuse:
;   * the 'ORG #C000' line was removed (the wrapper sets the origin);
;   * one downstream volume-scaling patch was removed (stock ROUT restored).
; Public entry points (see Bulba's notes below): CALL START+3 with HL = module
; address to init, CALL START+5 once per 50 Hz frame, CALL START+8 to mute.
